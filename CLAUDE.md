...
每次呼叫 Gemini 前後，Claude 必須在回應中明確告知使用者：
...
範例：
> 🔍 **呼叫 Gemini Flash** — 查詢 Leaflet.js flyTo 參數文件
...
範例：
> ✅ **Gemini Flash 結果**：duration 建議 0.5–2 秒，easeLinearity 建議 0.25
> **Claude 判斷**：採用，與現有程式碼相容，已套用

若完全由 Claude 自行處理（未呼叫 Gemini），無需特別標注。
使用者若想知道「這個任務是誰處理的」，可以直接問，Claude 必須如實說明。
